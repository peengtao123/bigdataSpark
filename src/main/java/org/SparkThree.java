...
//        sqlDF.show();
        String sql = "select distinct a._c0 as user_id from views a " +
                "where a._c1=1629 and substring(a._c2,0,10)='2016-10-";
        ///前三家公司1629，517,58   5556715
        for(int i=1;i<=31;i++){
            Dataset<Row> sqlDF = spark.sql(sql+new DecimalFormat("00").format(i)+"'");
            sqlDF.cache();
            for(int j=i;j<=31;j++){
                String date=(new DecimalFormat("00").format(j)+"/"+new DecimalFormat("00")
                        .format(i));
